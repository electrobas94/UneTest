...
public:
    ...
    int ChangeVopros(quint16 nindex)//смена текущего вопроса
    {
        ...
        TestList->Current()->VoprosList->Current()->ReSetGroup();
    ...
    void OpenTestBD(QString tablename)
    {
        ...
        for(int i=0;i<contvop;i++)
        {
            ...
            for(int j=0;j<vopros.countOt;j++)//создаем ответы к вопросам
            {
                ...
                if(j==0)
                    test->VoprosList->Current()->OtwetList->Current()->GetRadio()->setChecked(true);
            }
        }
    ...
    bool OpenTest()//открыть тест(как онлайн так и локально)
    {
        ...
        try {// что бы прога не падала если экстренно закрываеть диалог
            ...
            for(int i=0;i<info.countvop;i++)
            {
                ...
                textVopros->setText(vopros.text);
                 ...
                 test->VoprosList->Current()->SetImage(vopros.image);
                 imgLabl->setPixmap(QPixmap::fromImage(vopros.image));
                ...
                qApp->processEvents();
                //test->VoprosList->Current()->ReSetGroup();
            }
    ...
    void SaveTest()
    {
        ...
        {
            //TestSavLod->SaveInBD(TestList->Current());
        }
        else
        {
            try {// что бы прога не падала если экстренно закрываеть диалог
                ...
                if(str=="")
                    return;
                //TestSavLod->SaveInFile(TestList->Current(),str);
    ...
    void LoadImage()// загрузка нового изображения к вопросу
    {
        if(TestList->Count()==0)
            return;
    ...
    void RetTestList(QComboBox *list)
    {
        list->clear();
        QStringList *tmp=new QStringList();
    ...
    QString RetKommentTest(quint16 index)
    {
        return TestSavLod->SUBD->RetFirstItem(TestSavLod->SUBD->RetTableName(index), 5);
    }
};
