...
public:
    ...
    void ReadPravils(MyTest *test)
    {
        ...
        for(int i=0;i<counP;i++)
        {
            ...
            for(int j=0;j<counI;j++)
            {
                ...
                File.ReadDataBlock(tmp3->attrib);
    ...
    {
        File.ReadDataBlock(vopros.text);//пишем текст вопроса
        vopros.text=Crypto->Crypt(vopros.text);

        File.ReadDataBlock(vopros.image);
    ...
    bool SaveInBDRezult(MyTest *testCur)
    {
        QString tablename=CalkTableNameR();
        SUBD->CreteTableTest(tablename);

        SUBD->ApendDataInTest(tablename, testCur->TestName);
        SUBD->ApendDataInTest(tablename, testCur->FIO[0]);
        SUBD->ApendDataInTest(tablename, testCur->FIO[1]);
        SUBD->ApendDataInTest(tablename, testCur->FIO[2]);
        SUBD->ApendDataInTest(tablename, QString::number(testCur->kurs));

        SUBD->ApendDataInTest(tablename, QDate::currentDate().toString("dd/MM/yyyy"));

        qint32 tmppp=testCur->rezult->Count();
        SUBD->ApendDataInTest(tablename, QString::number(tmppp));

        testCur->rezult->First();
        for(int i=0;i<testCur->rezult->Count();i++)
        {
            SUBD->ApendDataInTest(tablename, testCur->rezult->Current()->head);
            SUBD->ApendDataInTest(tablename, testCur->rezult->Current()->info);
            SUBD->ApendDataInTest(tablename, QString::number(testCur->rezult->Current()->sum));
            SUBD->ApendDataInTest(tablename, QString::number(testCur->rezult->Current()->attrib));
...
private:
    ...
    QString CalkTableNameR()//вычислене имя новой таблицы(если ретурн "" - значит перезапись?)
    {
        QStringList tmp;
        if(!(SUBD->RetTablesRezult(&tmp)))//получаем список таблиц
            return "-1";
        ...
        tablname=tmp.last();
        tablname.remove(0,1);
        tablname="r"+QString::number(tablname.toInt()+1);
        return tablname;//имя t+kolwo
